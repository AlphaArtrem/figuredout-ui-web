...
| 6. Chat and navigation | 2026-09-23 (`ds/chat-nav`) | A full-height conversation (`ChatPane`, `MessageList`, `Composer`…) that anchors to the bottom and keeps the reader's place; ⌘K `CommandPalette`, `SegmentedControl`, `BottomNav`, `Popover` + `NotificationList` — all in Storybook, light and dark |
